...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
  ...
  <div class="bg-white shadow-md rounded-lg p-8 max-w-md w-full">
    <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Update Post</h2>

    @if (session('success'))
      <h2 class="text-green-600 text-center font-semibold mb-4">{{ session('success') }}</h2>
    @endif

    <form action="{{ route('update', $post->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
      @csrf
      ...
      <div>
        ...
        @if($post->image)
            <img src="{{ asset('images/' . $post->image) }}" alt="Current Image" class="mt-2 w-20 h-20 object-cover rounded-md">
        @endif
      </div>
